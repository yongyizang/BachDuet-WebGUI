  ...
  methods: {
    ...
    keyUp(noteInput, AI){
      ...
      if (this.currentNotes[selector] && this.currentNotes[selector].length){
        ...
        note.plane.scale.y = Math.max(this.camera.position.y - note.position, 5);
        note.plane.position.y = this.$refs.canvas.clientHeight + KeyboardUIHeight + note.position + note.plane.scale.y / 2;
      }
    },
  }
}
