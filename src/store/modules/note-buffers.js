import Vue from "vue";
import { createRange, durationMap } from "../../library/music"
...
const state = {
    ...
    // Midi tracks for Human and AI in which MIDI events will be stored
    track_human: null,
    track_AI: null,
    wait_AI : 0,
    wait_Human : 0
...
const getters = {
    ...
    getMidiTrackAI (state){
        return state.track_AI;
    },
    getMidiTrackHuman (state){
        return state.track_human;
    }
    ...
    newAiPrediction ({ commit, state, getters }, aiPrediction) {
        ...
        if (midi == 0) {
            if (state.lastAINote.midi == 0){
                ...
                // first add the lastAINote in the track_AI
                commit("addMidiEventToTrack", {"user" : "AI",
                                                "midi" : state.lastAINote.midi,
                                                "dur" : state.lastAINote.dur,
                                                "wait" : state.wait_AI})
            ...
            if (artic == 1){
                // first add the lastAINote in the track_AI
                // check if the lastAINote is rest (midi=0)
                if (state.lastAINote.midi == 0){
                    // pass
                }
                else {
                ...
                // The assertion fails when the AI generates notes with artic=0
    ...
    newHumanInputQuantized ({ commit, state, getters }, args) {
        ...
        if (args.midi === 0){
            if (state.lastHumanNote.midi === 0){
                ...
                // first add the lastAINote in the track_AI
                commit("addMidiEventToTrack", {"user" : "Human",
                                                "midi" : state.lastHumanNote.midi,
                                                "dur" : state.lastHumanNote.dur,
                                                "wait" : state.wait_Human})
            ...
            if (args.artic == 1){
                // first add the lastHumanNote in the track_human
                // check if the lastHumanNote is rest (midi=0)
                if (state.lastHumanNote.midi == 0){
                    // pass
                }
                else {
...
const mutations = {
    ...
    initializeMidiTracks (state) {
        state.track_AI = new MidiWriter.Track();
        state.track_human = new MidiWriter.Track();
        state.track_human.addEvent(new MidiWriter.ProgramChangeEvent({instrument: 1}));
        state.track_AI.addEvent(new MidiWriter.ProgramChangeEvent({instrument: 1}));
        state.wait_AI = 0;
        state.wait_Human = 0;
    },
    addMidiEventToTrack (state, args) {
        // "user" : "AI",
        // "midi" : state.lastAINote.midi,
        // "dur" : state.lastAINote.dur,
        // "wait" : state.wait
        // split the duration in whole notes

        if (args.dur > 0) {
            var wholes = Math.floor(args.dur / 16);
            var remain = args.dur % 16;
            var duration_string = [];
            for (const x of Array(wholes).keys()) {
                duration_string.push(durationMap[16]);
            }
            duration_string.push(durationMap[remain]);
            console.log(duration_string);
            const note = new MidiWriter.NoteEvent({pitch: Note.fromMidiSharps(args.midi), 
                                                    duration: duration_string});
            if (args.user == "AI"){
                state.track_AI.addEvent(note);
            }  
            else {
                state.track_human.addEvent(note);
            }     
        }                              
